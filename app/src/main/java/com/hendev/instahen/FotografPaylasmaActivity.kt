...
class FotografPaylasmaActivity : AppCompatActivity() {
    ...
    fun paylas(view: View){
        ...
        if (secilenGorsel != null){
            gorselReference.putFile(secilenGorsel!!).addOnSuccessListener { taskSnapshot ->
                println("Yüklendi")
                val yuklenenReferance = FirebaseStorage.getInstance().reference.child("images").child(gorselIsmi)
                yuklenenReferance.downloadUrl.addOnSuccessListener { uri ->
                    val downloadUri = uri.toString()
                    val kullanici = auth.currentUser!!.email.toString()
                    val yorum = edtYorum.text.toString()
                    val tarih = Timestamp.now()

                    val postHashMap = hashMapOf<String, Any>()
                    postHashMap.put("gorselurl",downloadUri)
                    postHashMap.put("kullanici",kullanici)
                    ...
                    database.collection("Post").add(postHashMap)
                        .addOnCompleteListener { task ->
                            ...
                            Toast.makeText(applicationContext,exp.localizedMessage,Toast.LENGTH_LONG).show()
                        }
                }.addOnFailureListener { exp ->
                    Toast.makeText(applicationContext,exp.localizedMessage,Toast.LENGTH_LONG).show()
                }
    ...
    fun gorselSec(view: View){
        if(ContextCompat.checkSelfPermission(this,Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED){
            //0-İzin verilmemisse istenecek
            ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.READ_EXTERNAL_STORAGE),1)
        }else{
            // 1-izin zaten verilmisse yapıalcak olanlar
